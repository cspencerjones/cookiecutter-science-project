...
```
Build the environment with `conda env create -f docs/environment.yml`, and activate it with `conda activate <repo>_docs`.
> For an even faster build try out `mamba env create -f docs/environment.yml` instead.
